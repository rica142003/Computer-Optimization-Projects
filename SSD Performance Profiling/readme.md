...
By conducting controlled experiments—ranging from zero-queue baselines and block-size sweeps to read/write mix variation, queue-depth scaling, and tail-latency characterization—the project provides a complete framework for analyzing device performance across multiple dimensions. The overall goal is not only to measure peak bandwidth or IOPS, but also to understand where diminishing returns occur, how workload patterns affect observable behavior, and what implications tail latencies have for real-world service-level requirements.

# Methodology

## Data Hygiene and Setup

The experimental methodology was designed to ensure that measured performance reflects intrinsic device-level behavior. All I/O was issued using direct access (`direct=1`) with 4 KiB alignment to bypass the page cache and eliminate partial-block penalties. Tests were conducted directly against a raw device rather than a mounted filesystem, which isolates block-level performance characteristics. For the SSD profiling experiments, a dedicated USB flash drive was used as the target medium. The drive was preconditioned before random-write workloads by filling the target range with random data, ensuring steady-state behavior; TRIM/discard operations were noted to avoid transient artifacts. Device temperature was stabilized through consistent airflow, with temperatures logged to detect thermal throttling.

On the host side, the system environment was controlled for reproducibility. The experiments were performed on a 12th Gen Intel(R) Core(TM) i7-1260P CPU (x86-64 architecture) running Linux. To reduce variability, the CPU frequency governor was set to `performance`, pinning all cores to the maximum clock speed of 4.9 GHz. This prevented frequency scaling and reduced jitter from background scheduling. Tests were run in a partition isolated from the main OS workload to minimize interference, with background services and other I/O activity suppressed. Where relevant, cores were isolated and workloads pinned using `taskset` to prevent thread migration across cores.

Workload patterns were standardized to maintain consistency across runs. Incompressible payloads were used to avoid artificial gains from controller-side compression. Trial order was randomized and each configuration was repeated multiple times; results are reported as mean ± standard deviation, alongside percentile latencies (p95/p99) to capture tail behavior. Interface ceilings were considered during interpretation, with reference bandwidths for USB 3.x, SATA, and PCIe explicitly documented. All software and hardware environments—including FIO version, Linux kernel build, CPU/SSD model, and device interface—were logged to ensure full reproducibility. This emphasis on controlled methodology and data hygiene ensures that reported results reflect device-level limits rather than uncontrolled host or environmental effects.

### Known Limitations

Several limitations arise from the choice of hardware and platform. First, the experiments were conducted on a USB flash drive, which differs significantly from enterprise-grade SATA or NVMe SSDs. The USB bus introduces higher host-side latency, additional variance due to shared controller paths, and stricter throughput ceilings that cap observed performance below typical SSD specifications. Second, consumer flash drives often rely on lightweight controllers with minimal DRAM buffers and aggressive wear-leveling or garbage collection policies, which can cause unpredictable latency spikes during steady-state testing. Third, the laptop-class i7-1260P processor, while pinned to its maximum frequency, is subject to thermal constraints that may throttle sustained workloads despite airflow stabilization. Together, these factors mean that while the methodology isolates device behavior under controlled conditions, the absolute performance levels reported here should be interpreted in the context of a consumer USB device rather than a dedicated NVMe SSD. As such, conclusions about throughput ceilings and latency scaling are internally consistent but not directly comparable to enterprise-class storage benchmarks.
...
The Intel Data Center NVMe SSD D7-P5600 (1.6 TB) lists ≈130K 4 KiB random write IOPS, while the USB flash drive measured only ~300–350 IOPS at QD=1 (≈1 MB/s). This gap of nearly three orders of magnitude is expected. The D7-P5600 runs over PCIe 4.0×4, giving ~7.5 GB/s link bandwidth, deep queues, and a controller with many NAND channels and large DRAM caches. By contrast, USB flash exposes a much slower interface, almost no internal parallelism, and minimal caching. The USB controller reaches saturation quickly, so higher queue depths bring no IOPS gain, while NVMe devices thrive on QD=32 or higher.

Controller and firmware policy also explain the difference. Enterprise SSDs use over-provisioning, write coalescing, and carefully managed garbage collection to sustain high steady-state random write IOPS. Small writes are absorbed into caches and reorganized into large block writes internally, keeping write amplification low. USB flash lacks these features. Each 4 KiB write may trigger a larger erase-program cycle, leading to heavy write amplification and multi-millisecond latencies. Background wear-leveling and garbage collection can further increase tail latency, whereas enterprise devices schedule these processes more predictably.

Other factors include data patterns and host interface behavior. Enterprise specs are measured with controlled, steady-state random data, while consumer devices may rely on SLC caching or compressible data for short bursts. If incompressible data and direct I/O are used, the USB results reflect true steady-state cost. Interface ceilings also play a role: PCIe supports massive command parallelism with low driver overhead, while USB adds protocol delays and often allows only shallow queues. Together, these limits explain why the D7-P5600 sustains ≈130K IOPS while the USB device stalls near ~300.
